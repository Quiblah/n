...
// Takes distance to move, speed to move, and at what interval scans should be.
int MoveScan(int distance_mm, int speed, int interval_ms){
        //time is how long we need to move to achieve the distance.
        float time_ms = ((float)distance_mm / speed) * 1000;
        //From time and interval the number of scans is worked out.
        float num_scans = time_ms/interval_ms;
        //ADC is the most recent reading taken from the sensor
        int ADC = 0;
        int i = 0;
        int j = 0;
        //ADC2 is the previous reading from the sensor
        int ADC2 = 0;
  
        for (i; i<num_scans;i++){
                //Move forwards for the time between scans.
                SetSpeed(speed,speed);
                DelayMs(interval_ms);
                //Get a reading from the sensor
                ADC = ADC_Read(0);
                //Exit if an object is found
                if(ADC < 266 && ADC > 204 && ADC2 > ADC){
                        return 1;
                }
                //Extra scans left-right to increase scanned area
                if (i % 100 == 0) {
                        SetSpeed(0, 0);
                        for(j=0; j<=10; j++) {
                                if(ADC < 266 && ADC > 204 && ADC2 > ADC){
                ...
                //keeping the previous sensor reading.
                ADC2 = ADC;
        }
        return 0;
}
...
void ZigZag(int speed, int sections)
{
        ...
        {
                //Follow the path for one section.
